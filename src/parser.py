...
#   Updated: 2026/09/22 11:42:41 by horarivo           ###   ########.fr      #
#                                                                             #
# ########################################################################### #
...
class Parser:
    ...
    def _parse_connection_line(self, line: str, line_num: int) -> None:
        ...
        if name1 not in self._zones:
            raise ParseError(
                line_num, f"connection references undefined zone: {name1!r}"
            )
        if name2 not in self._zones:
            raise ParseError(
                line_num, f"connection references undefined zone: {name2!r}"
            )
        if name1 == name2:
            raise ParseError(
                line_num, f"connection cannot link a zone to itself: {name1!r}"
            )
